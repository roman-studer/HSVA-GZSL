...
class Model(nn.Module):
    ...
    def __init__(self, hyperparameters, split: int = None):
        super(Model, self).__init__()
        self.gen_warmup = hyperparameters['gen_warmup']
        ...
        enc_params = list(self.encoder['resnet_features'].parameters()) + list(
            self.encoder[self.auxiliary_data_source].parameters())
        dec_params = list(self.decoder['resnet_features'].parameters()) + list(
            self.decoder[self.auxiliary_data_source].parameters())
    ...
    def train_classifier(self, current_epoch, logger):
        ...
        best_gzsl_acc = 0
        best_zsl_acc = 0
        best_unseen, best_seen, best_H = 0, 0, 0
        if self.selected_cls == 'softmax':
            ...
            if self.generalized:
                ...
                logger.log_metrics({'epoch': current_epoch, 'unseen': best_unseen, 'seen': best_seen, 'h': best_H})
    ...
    def gen_update(self, img, att, label, unseen_att):
        ...
        reconstruction_factor_cross = torch.cuda.FloatTensor(
        ...
        self.distance_seen_seen = torch.sqrt(torch.sum((mu_img - mu_att) ** 2, dim=1) + \
                                             torch.sum(
                                                 (torch.sqrt(logvar_img.exp()) - torch.sqrt(logvar_att.exp())) ** 2,
                                                 dim=1)).sum()

        self.distance_seen_unseen_att = torch.sqrt(torch.sum((mu_att - mu_unseen_att) ** 2, dim=1) + \
                                                   torch.sum((torch.sqrt(logvar_att.exp()) - torch.sqrt(
                                                       logvar_unseen_att.exp())) ** 2, dim=1)).sum()

        self.distance_seen_unseen_img = torch.sqrt(torch.sum((mu_img - mu_unseen_att) ** 2, dim=1) + \
                                                   torch.sum((torch.sqrt(logvar_img.exp()) - torch.sqrt(
                                                       logvar_unseen_att.exp())) ** 2, dim=1)).sum()
    ...
    def train_vae(self, logger):
        ...
        for epoch in range(0, self.nepoch):
            ...
            for iters in range(0, self.dataset.ntrain, self.batch_size):
                ...
                distance, loss_gen = self.gen_update(data_from_modalities[0], data_from_modalities[1], seen_label,
                                                     data_from_unseen[0])
                if i % 10 == 0:
                    # print('epoch ' + str(epoch) + ' | iter ' + str(i) + '\t'+
                    # ' | loss_dis ' +  str(loss_dis)[7:15] + ' | loss_gen ' +  str(loss_gen)[8:15])
                    print('epoch ' + str(epoch) + ' | iter ' + str(i) + '\t' + ' | loss_gen ' + str(loss_gen)[
                                                                                                8:14] + ' | distance ' + str(
                        distance)[7:13])
